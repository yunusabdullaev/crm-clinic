                    ...
                    <div className="modal-overlay" onClick={closeModal}>
                        <div className="modal" onClick={(e) => e.stopPropagation()} key={modalKey}>
                            ...
                            <form onSubmit={handleCreateUser}>
                                <div className="form-group"><label>{t('boss.phoneNumber')}</label><div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ padding: '8px 12px', background: 'var(--bg)', borderRadius: 6, fontWeight: 500, color: 'var(--text-muted)' }}>+998</span><input className="input" type="tel" style={{ flex: 1 }} value={userForm.phone} onChange={(e) => setUserForm({ ...userForm, phone: e.target.value.replace(/[^0-9]/g, '').slice(0, 9) })} maxLength={9} required /></div></div>
